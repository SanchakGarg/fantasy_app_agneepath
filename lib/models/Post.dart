...
class Post extends amplify_core.Model {
  ...
  const Post._internal({required this.id, required title, required rating, required status, comments, createdAt, updatedAt}): _title = title, _rating = rating, _status = status, _comments = comments, _createdAt = createdAt, _updatedAt = updatedAt;
  ...
  Post copyWith({String? title, int? rating, PostStatus? status, List<Comment>? comments}) {
    return Post._internal(
      id: id,
  ...
  Map<String, dynamic> toJson() => {
    'id': id, 'title': _title, 'rating': _rating, 'status': amplify_core.enumToString(_status), 'comments': _comments?.map((Comment? e) => e?.toJson()).toList(), 'createdAt': _createdAt?.format(), 'updatedAt': _updatedAt?.format()
  };
